...
export default function AdminCategories() {
  ...
  const fetchCategories = async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get("/categories", { params: { page, per_page: PAGE_SIZE } });
  ...
  const openEdit = (c) => { setNom(c.nom); setEditId(c.id); setError(""); setModal("edit"); };

  const handleSubmit = async () => {
    if (!nom.trim()) { setError("Le nom est obligatoire."); return; }
    try {
      if (modal === "add") await api.post("/categories", { nom });
      else await api.put(`/categories/${editId}`, { nom });
      setModal(null);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (categories.length === 1 && currentPage > 1) setCurrentPage((p) => Math.max(p - 1, 1));
      else fetchCategories(currentPage);
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-white" style={{ fontFamily: "'Orbitron', sans-serif" }}>Gestion des catégories</h1>
          <p className="text-gray-500 text-xs sm:text-sm mt-1">Ajouter, modifier ou supprimer des catégories</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            <table className="w-full min-w-max">
              ...
              <tbody>
                {categories.length === 0 ? (
                  <tr><td colSpan={2} className="px-4 py-6 text-center text-gray-500 text-sm">Aucune catégorie trouvée</td></tr>
                ) : (
                  categories.map((c) => (
                    <tr key={c.id} className="border-b border-white/3 hover:bg-white/2 transition-colors">
                      <td className="px-4 py-3 text-sm font-semibold text-white">
                        <span className="px-3 py-1 rounded-lg bg-pink-500/15 text-pink-400">{c.nom}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button onClick={() => openEdit(c)} className="text-xs px-2 sm:px-3 py-1.5 rounded-lg bg-violet-500/15 text-violet-400 hover:bg-violet-500/25 cursor-pointer transition-colors min-h-8 flex items-center gap-1" aria-label="Modifier la catégorie">
                            <i className="bi bi-pencil" aria-hidden="true"></i><span className="hidden sm:inline">Modifier</span>
                          </button>
                          <button onClick={() => handleDelete(c.id)} className="text-xs px-2 sm:px-3 py-1.5 rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/25 cursor-pointer transition-colors min-h-8 flex items-center gap-1" aria-label="Supprimer la catégorie">
            ...
            {totalPages > 1 && (
              <div className="mt-4 p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-xs text-gray-400">Affichage {displayedStart} - {displayedEnd} sur {total} catégories</div>
        ...
        <Modal title={modal === "add" ? "Ajouter une catégorie" : "Modifier la catégorie"} onClose={() => setModal(null)}>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="text" value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Ex: RPG" className="bg-[#0F0F1A] border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white outline-none focus:border-violet-500/50" />
            </div>
